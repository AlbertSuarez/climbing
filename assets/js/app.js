...
const INDOOR_GYMS = ['Sharma Climbing BCN Gavá', 'Klättercentret Solna'];
...
function processData() {
  ...
  boulderData = filteredData.filter(
    (row) => row['Route Gear Style'] === 'Boulder'
  );

  // Sort by date (newest first)
  const sortByDate = (a, b) =>
    new Date(b['Ascent Date']) - new Date(a['Ascent Date']);
...
async function updateStats() {
  ...
  document.getElementById('hardest-boulder').textContent =
    hardestBoulder || 'N/A';

  // Unique crags count
  const uniqueCrags = new Set(
    [...sportData, ...boulderData].map((row) => row['Crag Name'])
  );
...
function createPerformanceChart(type) {
  ...
  data.forEach((row) => {
    ...
    if (gradeSystem[grade]) {
      ...
      if (
        type === 'sport' &&
        (ascentType === 'Onsight' || ascentType === 'Flash')
      ) {
  ...
  const sortedGrades = Object.keys(gradeStats).sort(
    (a, b) => gradeSystem[a] - gradeSystem[b]
  );

  // Prepare data for stacked bar chart
  const onsightFlashData = sortedGrades.map(
    (grade) => gradeStats[grade].onsightFlash
  );
  const redpointData = sortedGrades.map(
    (grade) => gradeStats[grade].total - gradeStats[grade].onsightFlash
  );
...
function updateCragList(type) {
  ...
  if (container) {
    container.innerHTML = sortedCrags
      .map(([crag, count]) => {
...
function updateMonthlyActivityForType(type) {
  ...
  const monthAverages = Object.entries(monthCounts)
    .map(([month, total]) => {
  ...
  if (container) {
    container.innerHTML = monthAverages
      .map(
        ({ month, total, average, yearsActive }) => `
...
function createProgressionChart(type) {
  ...
  data.forEach((row) => {
    ...
    if (gradeValue) {
      if (
        ...
        gradeValue > monthlyProgress[monthKey].value
      ) {
        monthlyProgress[monthKey] = {
          value: gradeValue,
          grade: grade,
  ...
  sortedMonths.forEach((month) => {
    ...
    progressData.push({
      x: month,
      y: cumulativeMax,
      grade: Object.keys(gradeSystem).find(
        (key) => gradeSystem[key] === cumulativeMax
      ),
    });
  });
        ...
        y: {
          ...
          ticks: {
            callback: function (value) {
              const grade = Object.keys(gradeSystem).find(
                (key) => gradeSystem[key] === value
              );
              return grade || value;
            },
            color: '#718096',
...
function updateRoutesList(type) {
  ...
  hardestContainer.innerHTML = hardestRoutes
    .map((route) => createRouteItem(route))
    .join('');

  // Update recent routes
  const recentContainer = document.getElementById(`${type}-recent-routes`);
  recentContainer.innerHTML = recentRoutes
    .map((route) => createRouteItem(route))
    .join('');
...
function createRouteItem(route) {
  ...
  const renderStars = (starText) => {
    ...
    const emptyStars =
      starCount < 3
        ? `<span class="empty-star">${'☆'.repeat(3 - starCount)}</span>`
        : '';
...
function initializeScrollAnimations() {
  ...
  const animatedElements = document.querySelectorAll(
    '[class*="scroll-animate"]:not(.github-badge)'
  );
  animatedElements.forEach((element) => {
    // Double check to exclude GitHub badge and its children
    if (
      !element.classList.contains('github-badge') &&
      !element.closest('.github-badge')
    ) {
...
function reinitializeAnimationsForActiveSection() {
  ...
  setTimeout(() => {
    ...
    const containers = activeSection.querySelectorAll(
      '.stats-container, .chart-container, .routes-container'
    );
